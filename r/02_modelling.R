...
#                          num_leaves = c(32, 63, 127), #c(7, 15, 31, 63), # c(31, 63, 127, 255),
#                          # max_depth = 14,
#                          min_data_in_leaf = c(10, 50, 100),
#                          lambda_l1 =  3:5, #, c(0, 0.5, 1),
#                          lambda_l2 = 1:4, # c(0, 0.1, 0.5, 1:6),
#                          min_sum_hessian_in_leaf = c(0, 0.01, 0.02), #c(0, 0.001, 0.1),
#                          feature_fraction = c(0.6, 0.8, 1), #c(0.6, 0.8), ## 0.8, #seq(0.4, 1, by = 0.2),
#                          bagging_fraction = 1, #c(0.8, 1), # seq(0.4, 1, by = 0.2),
#                          bagging_freq = 1,
#                          nthread = 4,
#                          monotone_constraints = "0,0,-1,1,0")
...
explainer_lgb <- explain(fit_lgb, data = valid[, x], y = valid[[y]], label = "Tree booster", 
                         link = exp,
                         predict_function = function(model, X) predict(model, prep_lgb(X)))

explainers <- list(explainer_lm, explainer_rf, explainer_lgb)

# Model performance
perf <- sapply(explainers, get_performance)
...
save(perf, varImp, most_relevant, eff, file = "rdata/models.RData")
